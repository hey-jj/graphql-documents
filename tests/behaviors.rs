...
fn comments_and_commas_are_ignored() {
    let out = canonicalize(&parse("query A { # leading comment\n c, b, a }").unwrap());
    assert_eq!(out, "query A { a b c }");
}

#[test]
fn anonymous_query_keeps_shorthand() {
    let out = canonicalize(&parse("{ b a }").unwrap());
    assert_eq!(out, "{ a b }");
}
...
fn anonymous_subscription_keeps_keyword() {
    ...
    let out = canonicalize(&parse("subscription { b a }").unwrap());
...
fn stable_order_for_equal_keys() {
    ...
    let out = canonicalize(&parse("query A { ... on Q { a } ... on Q { a } }").unwrap());
    assert_eq!(out, "query A { ... on Q { a } ... on Q { a } }");
}

#[test]
fn variable_definitions_and_default_values_round_trip() {
    let out = canonicalize(&parse("query A($b: [Int!] = [1, 2], $a: String!) { f }").unwrap());
